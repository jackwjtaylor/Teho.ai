...
<reminder_summary>🔔 Reminder set: I'll remind you to test the authentication fix in 2 hours</reminder_summary>
...
\`\`\``,
    ...
    const timeMatch = dateResult.match(/<TIME>(.*?)<\/TIME>/);
    if (!timeMatch) {
      console.error('❌ Invalid date format in AI response');
